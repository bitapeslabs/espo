...
use crate::modules::ammdata::storage::{
    AmmDataProvider, GetListEntriesDescParams, GetRawValueParams, SetBatchParams, decode_candle_v1,
    decode_full_candle_v1, decode_u128_value, encode_tvl_point_v1, encode_u128_value,
};
...
use crate::modules::ammdata::utils::index_tvl::{PoolAnchorInput, pool_anchor_point};
...
impl<'a> BackfillWalker<'a> {
    ...
    /// Value one side of a pool in sats, mirroring `derive_pool_metrics`.
    fn side_tvl_sats(
        &self,
        ...
        if let Some(unit) = canonical_quote_units.get(token).copied() {
            return crate::modules::ammdata::canonical_quote_amount_tvl_sats(amount, unit, btc_usd)
                .unwrap_or(0);
        }
    ...
    fn token_price_usd(&mut self, token: &SchemaAlkaneId, block_ts: u64) -> u128 {
            ...
            .get_list_entries_desc(GetListEntriesDescParams { blockhash: StateAt::Latest, prefix })
        {
            for (key, value) in res.entries {
                if !candle_key_at_or_before(&key, bucket) {
                    continue;
                }
    ...
    fn load_candle_at_or_before(
        ...
        for (key, value) in res.entries {
            ...
            return decode_full_candle_v1(&value).ok();
        }
        None
    }
...
fn candle_key_at_or_before(key: &[u8], bucket: u64) -> bool {
    let Some(ts_bytes) = key.rsplit(|&b| b == b':').next() else { return false };
    let Ok(ts_str) = std::str::from_utf8(ts_bytes) else { return false };
...
mod tests {
    ...
    #[test]
    fn candle_key_cutoff_compares_the_trailing_bucket() {
        assert!(candle_key_at_or_before(b"fc1:2:0:10m:1700000000", 1_700_000_600));
        assert!(candle_key_at_or_before(b"fc1:2:0:10m:1700000600", 1_700_000_600));
        assert!(!candle_key_at_or_before(b"fc1:2:0:10m:1700001200", 1_700_000_600));
    }
}
