...
fn docs_modules() -> Vec<ModuleDoc> {
    vec![
        ...
        ModuleDoc {
            ...
            intro: "Read-only storage primitives that let a remote espo explorer (configured with explorer_espo_rpc_host) fulfil its server-side rendering data needs against this instance. Every module getter bottoms out in these Mdb primitives, so serving them powers the entire remote explorer. Registered only when enable_internal_rpc is true, which also requires internal_rpc_key: every request must carry the configured key as \"auth\" in its params or it is rejected with unauthorized (the remote explorer sends it automatically when explorer_espo_rpc_key is set). The methods still expose raw namespace reads and scans, so keep them off untrusted public endpoints regardless. All keys and values are hex encoded; prefix is a module namespace like \"essentials:\"; the optional blockhash (32 hex bytes, internal byte order) reads versioned namespaces at that block's state instead of latest.",
            methods: vec![
                rpc_doc(
                    "internal.mdb_get",
                    ...
                    json!({ "auth": "<internal_rpc_key>", "prefix": "essentials:", "key": "2f696e6465785f686569676874" }),
                ...
                rpc_doc(
                    ...
                    json!({ "auth": "<internal_rpc_key>", "prefix": "essentials:", "keys": ["6b65792d61", "6b65792d62"] }),
                ...
                rpc_doc(
                    ...
                    json!({ "auth": "<internal_rpc_key>", "prefix": "essentials:", "scan_prefix": "686f6c646572732f" }),
                ...
                rpc_doc(
                    ...
                    json!({ "auth": "<internal_rpc_key>", "prefix": "essentials:", "start": "61", "end": "62" }),
                ...
                rpc_doc(
                    ...
                    json!({ "auth": "<internal_rpc_key>", "prefix": "essentials:", "start": "61", "end": "62", "offset": 0, "limit": 100, "reverse": true }),
                    json!({ "ok": true, "entries": [["6161", "01"]] }),
                ),
                rpc_doc(
                    ...
                    json!({ "auth": "<internal_rpc_key>", "height": 946000 }),
